...
class Transformer(object):
	...
	def findCenter(self):
		arr = np.array([b.coords() for b in self.blocks])
		length = arr.shape[0]
		sum_x = np.sum(arr[:, 0])
		sum_y = np.sum(arr[:, 1])
		return (sum_x/length, sum_y/length)
	def rotate(self, degrees=90, pivot='d'):
		theta = np.deg2rad(-degrees)
		if pivot=='d':
			xo, yo = self.findCenter()
		else:
			xo, yo = pivot
		for b in self.blocks:
			xb, yb = b.coords()

			xr = np.cos(theta)*(xb-xo)-np.sin(theta)*(yb-yo)   + xo
			yr = np.sin(theta)*(xb-xo)+np.cos(theta)*(yb-yo)  + yo

			b.setCoords(xr,yr)
			b.rotate(degrees)
		return self
	def moveTo(self, x=0, y=0):
		b1x, b1y = self.blocks[0].coords()

		x = x-b1x
		y = y-b1y
		for b in self.blocks:
			xo, yo = b.coords()
			b.setCoords(x+xo, y+yo)
		return self	
	...
	def copyAll(self):
		blks = []
		for b in self.blocks:
			blks.append(Block(**b.json))
		self.blocks = blks
		return self
	def addToLevel(self, lev):
		lev.addBlocks(*self.blocks)
		return self

	def appendTransformer(self, trans):
		self.blocks+=trans.blocks
		return self

	def duplicate(self):
		return self.__class__(*self.blocks).copyAll()

	def clear(self):
		self.blocks = []
		return self
	def scale(self, X=2, origin=[0,0]):
		Px, Py = origin
		for b in self.blocks:
			Ax, Ay = b.coords()
			...
			b.setCoords(sx,sy)
		return self
	def removeOverlaps(self, cdist=8, crot=5, depth=10):
		c=0
		tmp = self.blocks.copy()
		tmp.sort(key=lambda block: block.coords()[0])
		print(f"Before: {len(tmp)}")
		while c<len(tmp):
			block1 = tmp[c]

			surrounding = tmp[max(0, c-depth):min(len(tmp),c+depth)]

			for block2 in surrounding:
				if block1==block2:
					continue
				dist = np.sqrt((block2.coords()[0] - block1.coords()[0])**2 + (block2.coords()[1] - block1.coords()[1])**2)
				rotDif = abs(float(block1['rotation']) - float(block2['rotation']))

				if dist<cdist and rotDif<crot:
					tmp.remove(block1)
					break
			#print("num is at %i"%c,end="\r")
			c+=1
...
class DynamicGroup(object):
	def __init__(self, lvl, groupid):
		self.lev = lvl
		self.groupid = int(groupid)
	@staticmethod
	def addGroupToBlock(b, group):
		b['group_ids'] = b['group_ids']+f',{group}' if len(b['group_ids'])>0 else str(group)
	@classmethod
	def new(cls, lvl, *blocks):
		n = cls(lvl, lvl.getfGroupID())
		for b in blocks:
			cls.addGroupToBlock(b,n.groupid)
		return n
	def addBlocks(self, *blocks):
		for b in blocks:
			DynamicGroup.addGroupToBlock(b, self.groupid)
		return self
	def initialize(self):
		blocks = self.lev.blocksInGroup(self.groupid)
		self.x1, self.y1 = Transformer(*blocks).findCenter()
		self.theta = 0
		return self
	def moveTo(self, x, y, movex=100, movey=100, duration=0.5):
		x1, y1 = self.x1, self.y1
		self.x1, self.y1 = movex, movey

		self.lev.addBlock(901,x, y,is_active_trigger_type=1,target_group_id=self.groupid, move_trigger_x_units=movex-x1, move_trigger_9_units=movey-y1, duration=duration, easing=0, easing_rate=2)
	def rotateTo(self, x, y, theta=90, duration=0.5):
		otheta = self.theta
		self.theta = theta
		if self.groupid>0:
			print(theta,theta-otheta)
		self.lev.addBlock(1346,x, y,is_active_trigger_type=1,target_group_id=self.groupid, rotation_degrees=otheta-self.theta, duration=duration, easing=0, easing_rate=2)
